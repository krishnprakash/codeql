...
static void bad3(wchar_t *str) {
    ...
    wchar_t *buffer = (wchar_t *)calloc(sizeof(wchar_t), wcslen(str)); // $ Alert
    wcscpy(buffer, str);
    free(buffer);
}
...
static void bad4(char *str) {
    // BAD -- Not allocating space for '\0' terminator
    char *buffer = (char *)realloc(0, strlen(str)); // $ Alert
    strcpy(buffer, str);
    free(buffer);
}
...
{
	{
		char *buffer1 = (char *)MyMalloc1(strlen(str)); // $ Alert // BAD (no room for `\0` terminator)
		strcpy(buffer1, str);
	}

	{
		char *buffer2 = (char *)MyMalloc2(strlen(str)); // $ Alert // BAD (no room for `\0` terminator)
		strcpy(buffer2, str);
	}
}
