...
void test4_1(const char *fileName) {
	...
	p = xmlReadFile(fileName, NULL, XML_PARSE_NOENT); // $ Alert // BAD (parser not correctly configured)
	if (p != NULL)
	{
		xmlFreeDoc(p);
	}
...
void test4_2(const char *fileName) {
	...
	p = xmlReadFile(fileName, NULL, XML_PARSE_DTDLOAD); // $ Alert // BAD (parser not correctly configured)
	if (p != NULL)
	{
		xmlFreeDoc(p);
	}
...
void test4_3(const char *fileName) {
	...
	p = xmlReadFile(fileName, NULL, XML_PARSE_NOENT | XML_PARSE_DTDLOAD); // $ Alert // BAD (parser not correctly configured)
	if (p != NULL)
	{
		xmlFreeDoc(p);
	}
...
void test4_6(const char *fileName) {
	...
	p = xmlReadFile(fileName, NULL, flags); // $ Alert // BAD (parser not correctly configured)
	if (p != NULL)
	{
		xmlFreeDoc(p);
	}
...
void test4_11(const char *ptr, int sz) {
	...
	p = xmlReadMemory(ptr, sz, "", NULL, XML_PARSE_DTDLOAD); // $ Alert // BAD (parser not correctly configured)
	if (p != NULL)
	{
		xmlFreeDoc(p);
	}
