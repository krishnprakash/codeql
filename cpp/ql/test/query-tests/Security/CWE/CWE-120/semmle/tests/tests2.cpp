...
void tests2() {
  ...
  wcscpy(buffer, L"12"); // $ Alert[cpp/very-likely-overrunning-write] // BAD: buffer overflow
  free(buffer);

  buffer = (wchar_t *)malloc(3 * sizeof(wchar_t));
  wcscpy(buffer, L"12"); // GOOD
  wcscpy(buffer, L"123"); // $ Alert[cpp/very-likely-overrunning-write] // BAD: buffer overflow
  free(buffer);

  buffer = (wchar_t *)realloc(0, 4 * sizeof(wchar_t));
  wcscpy(buffer, L"123"); // GOOD
  wcscpy(buffer, L"1234"); // $ Alert[cpp/very-likely-overrunning-write] // BAD: buffer overflow

  buffer = (wchar_t *)realloc(buffer, 5 * sizeof(wchar_t));
  wcscpy(buffer, L"1234"); // GOOD
  wcscpy(buffer, L"12345"); // $ Alert[cpp/very-likely-overrunning-write] // BAD: buffer overflow
  free(buffer);

  buffer = (wchar_t *)calloc(6, sizeof(wchar_t));
  wcscpy(buffer, L"12345"); // GOOD
  wcscpy(buffer, L"123456"); // $ Alert[cpp/very-likely-overrunning-write] // BAD: buffer overflow
  free(buffer);

  buffer = (wchar_t *)calloc(sizeof(wchar_t), 7);
  wcscpy(buffer, L"123456"); // GOOD
  wcscpy(buffer, L"1234567"); // $ Alert[cpp/very-likely-overrunning-write] // BAD: buffer overflow
  free(buffer);

  buffer = new wchar_t[8];
  wcscpy(buffer, L"1234567"); // GOOD
  wcscpy(buffer, L"12345678"); // $ Alert[cpp/very-likely-overrunning-write] // BAD: buffer overflow
  delete [] buffer;
}
