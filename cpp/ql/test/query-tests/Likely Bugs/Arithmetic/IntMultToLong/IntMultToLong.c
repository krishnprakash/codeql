...
void int_float(int i, int j, long long ll, float f, float g, double h, char c) {
    ...
    double v1_1 = f * g; // $ Alert // unsafe (float -> double)
    double v1_2 = f * (double)g; // safe

    double v2_1 = (i + j) * f; // $ Alert // unsafe (float -> double)
    ...
    double v7_1 = f * f; // $ Alert // unsafe (float -> double)
    ...
    double v13_2 = f * f * 2.0f; // $ Alert // unsafe (float -> double)

    long long v14_1 = i * (i + 2) + ll; // $ Alert // unsafe (int -> long long)
    long long v14_2 = i * (i + 2ll) * ll; // safe
    long long v14_3 = i * (i + (int)2ll) + ll; // $ Alert // unsafe (int -> long long)
}
...
void use_size_t(int W, int H)
{
	...
	malloc(W * H); // $ Alert // unsafe (int -> size_t)
...
void g3() {
    unsigned long ulong1, ulong2;
    ulong1 = global_i * global_i; // $ Alert // BAD
