...
{
	printf("\n"); // GOOD
	printf("\n", i); // $ Alert[cpp/too-many-format-arguments] // BAD (too many format arguments)

	printf("%i\n"); // $ Alert[cpp/wrong-number-format-arguments] // BAD (too few format arguments)
	printf("%i\n", i); // GOOD

	printf("%*s\n", str); // $ Alert[cpp/wrong-number-format-arguments] // BAD (too few format arguments)
	printf("%*s\n", i, str); // GOOD

	printf("%i %i %i\n", 1, 2); // $ Alert[cpp/wrong-number-format-arguments] // BAD (too few format arguments)
	printf("%i %i %i\n", 1, 2, 3); // GOOD

	// indexed format arguments
	printf("%2$i \n", 1); // $ Alert[cpp/wrong-number-format-arguments] // BAD (too few format arguments)
	printf("%2$i \n", 1, 2); // GOOD
	printf("%2$i \n", 1, 2, 3); // $ Alert[cpp/too-many-format-arguments] // BAD (too many format arguments)
	printf("%2$i %2$i %2$i \n", 1, 2); // GOOD
	printf("%2$02i %1$4.2f \n", 3.3333f, 6); // GOOD
	{
		int width, num;

		printf("%2$*1$d", 0, width, num); // $ Alert[cpp/too-many-format-arguments] // BAD (too many format arguments)
